...
[Build Pipeline](https://github.com/laozhen/realtime_fraud_detection_system/actions)

## Resilience Testing Report
Check [RESILIENCE_REPORT.md](docs/RESILIENCE_REPORT.md)

## Deliverable 
### [Test Coverage Report](docs/image/jacoco_test_report.png)
### [Helm Packages and Docker Images](https://github.com/laozhen?tab=packages&repo_name=realtime_fraud_detection_system)

### [Docs and Screenshots](docs)
...
Report is available in $projectRoot/fraud-detection-service/build/reports/jacoco/test/html/index.html
...
✅ **Benefits:**
- Decouples producer from consumer lifecycle
- Provides durability and at-least-once semantics
- Built-in retry with configurable visibility timeout
- Dead Letter Queue (DLQ) for poison messages
- Autoscaling consumers independently
- Cloud-managed (no operational overhead)
- Integrated CloudWatch metrics

❌ **Trade-offs:**
- Adds 2-5ms network latency per message
- Per-message cost (acceptable for financial services)
- No strict ordering (standard queue) - mitigated by idempotent processing
- Requires visibility timeout tuning
...
✅ **Benefits:**
- Parallel fraud detection (4 concurrent workers per pod)
- Keeps Disruptor event loop fast (no blocking operations)
- Natural backpressure via bounded queue (4,096 capacity)
...
```
# Business Metrics (FraudDetection/${environment})
fraud_detected_total.count{severity="HIGH|MEDIUM|LOW"}
transactions_sent_total.count
transactions_received_total.count
transactions_processed_total.count
transactions_cleared_total.count
rule_violations_total.count

# Performance Metrics
transaction_processing_duration_ms{p50,p90,p99}
sqs_publish_latency_seconds{p50,p90,p99}
transaction_publish_duration_seconds{p50,p90,p99}
high_latency_events_total.count

# Ring Buffer Metrics
ring_buffer_utilization_percent.value
ring_buffer_high_utilization_total.count

# Error Metrics
processing_errors_total.count{error_type="TRANSACTION_PROCESSING|PUBLISH_ERROR|SQS_PUBLISH_ERROR"}
...
# Container Insights Metrics (ContainerInsights namespace)
pod_cpu_utilization
